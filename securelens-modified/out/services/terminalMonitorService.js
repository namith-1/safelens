...
class TerminalMonitorService {
     ...
     */
    static handleTerminalData(event) {
    ...
    static checkAndAnalyzeErrors(buffer, terminal, termId) {
    ...
    static parseCustomError(buffer) {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            const isFalsePositive = lowerLine.includes('no error') ||
            ...
            if (containsErrorKeyword && !isFalsePositive) {
                ...
                for (const contextLine of contextLines) {
                    ...
                    if (match) {
                        ...
                        if (filePath.includes('node_modules') || filePath.includes('node:internal') || filePath.includes('sdk/'))
                            continue;
                        stackFrames.push({
                            file: filePath,
    ...
    static parsePythonTraceback(buffer) {
        const lines = buffer.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    ...
    static parseNodeTraceback(buffer) {
        const lines = buffer.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
        ...
        for (let i = headerIdx + 1; i < lines.length; i++) {
            ...
            if (!line.startsWith('at '))
                break; // Stop when stack trace ends
            const match = line.match(frameRegex);
            if (match) {
                if (match[2]) {
                    stackFrames.push({
    ...
    static parseGenericException(buffer) {
        const lines = buffer.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
        let errLineIdx = -1;
        let errType = 'RuntimeError';
    ...
    static async processException(exc, terminal) {
        const channel = historyService_1.HistoryService.getOutputChannel();
        channel.clear();
        const separator = '='.repeat(60);
        const workspaceFolders = vscode.workspace.workspaceFolders;
        const workspaceRoot = workspaceFolders?.[0]?.uri.fsPath || null;
        // A. Resolve relative paths
        const resolveRelPath = (fp) => {
        ...
        for (const f of exc.propagation.slice(0, 8)) {
            if (f.file.includes('node_modules') || f.file.includes('internal/'))
                continue;
        ...
        try {
            ...
            setTimeout(() => {
                sidebarProvider_1.SidebarProvider.postToWebview({
                    type: 'runtimeCrash',
                    payload: {
                        errorType: exc.type,
                        message: exc.message,
    ...
    static buildCrashTree(frames, originFile, workspaceRoot) {
        const rootNode = { name: 'Root', isFolder: true, children: {}, findings: [] };
    ...
    static printCrashTree(node, indent = '') {
        const lines = [];
    ...
    static async saveExceptionToHistory(exc, workspaceRoot) {
...
TerminalMonitorService.buffers = new Map();
//# sourceMappingURL=terminalMonitorService.js.map